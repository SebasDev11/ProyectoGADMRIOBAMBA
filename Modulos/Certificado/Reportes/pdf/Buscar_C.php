...
// Inicializar las variables de mensajes de error
$mensajeError = '';
$mensajeError2 = '';
$mensajeError3 = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($certificadoSeleccionado) || !empty($placa)) {
            $query = "SELECT certificado.*, usuario.Apellido, usuario.Nombre as Nombre_Usuario, usuario.Cedula, vehiculo.Numero_resolucion,
                    ...
                    WHERE certificado.Numero_certificacion = :certificadoSeleccionado
                    OR vehiculo.Placa LIKE :placa";
            ...
            if ($certificado) {
                CertificadoPDF::generarCertificado($certificado);
                 ...
                 // Mensaje JavaScript para mostrar la alerta
                $mensajeError = 'Certificado no encontrado.';
            }
        } else {
                $mensajeError2 = 'Por favor, ingrese la placa del vehículo.';
        }
    } catch (PDOException $e) {
        $mensajeError3 = "Error al obtener los datos del certificado: " . $e->getMessage();
    }
}
?>
<!DOCTYPE html>
...
<!-- Certificado -->
<div class="container mt-5 text-center"> <!-- Agregada la clase text-center para centrar el contenido -->
    <script>
        // SweetAlert solo se activa si hay mensajes de error
        <?php if (!empty($mensajeError)): ?>
            Swal.fire({
                icon: "error",
                title: "<?php echo $mensajeError; ?>",
                confirmButtonText: "Volver a intentar",
                confirmButtonColor: "#00214c"
            });
        <?php elseif (!empty($mensajeError2)): ?>
            Swal.fire({
                icon: "error",
                title: "<?php echo $mensajeError2; ?>",
                confirmButtonText: "Volver a intentar",
                confirmButtonColor: "#00214c"
            });
        <?php elseif (!empty($mensajeError3)): ?>
            Swal.fire({
                icon: "error",
                title: "<?php echo $mensajeError3; ?>",
                confirmButtonText: "Volver a intentar",
                confirmButtonColor: "#00214c"
            });
        <?php endif; ?>
    ...
    <div class="mb-3">
                ...
                <input type="text" name="placa" class="form-control" placeholder="Ingrese la placa del vehículo">
            </div>
            <button type="submit" class="btn btn-primary" style="background-color: #e83f44; color: #fff;">Generar Certificado</button>
    </form>
</div>
<div class="container">
    <a type="button" style="background: #223260; margin-left: 10px; margin-top: 4em;" href="../../../../Modulos/Certificado/Frm_Certificado.php?nivel=Administrador" id="add_button" class="btn btn-info">Regresar</a>
</div>
    <!-- Contact Us -->

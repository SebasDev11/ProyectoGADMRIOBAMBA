...
<div class="wrapper-main">
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
				...
				<?php
            		}elseif($nivel=="Socio")
					{
                    	...
                    	<a class="nav-link" href="#" id="navbarDropdownBlog" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Registro <i class="fas fa-sort-down"></i></a>
                    	<ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                      		<li><a class="dropdown-item" href="Modulos/Lider/Frm_Lider.php?nivel=Lider">Lider de Proyecto</a></li>
							<li><a class="dropdown-item" href="Modulos/Proyecto/Frm_Proyecto.php?nivel=Lider">Proyecto</a></li>
                      		<li><a class="dropdown-item" href="Modulos/Periodo/Frm_Periodo.php?nivel=Lider">Periodo</a></li>
							<li><a class="dropdown-item" href="Modulos/Componente/Frm_Componente.php?nivel=Lider">Componente</a></li>
                        </ul>
                    	</li>';
					}
          		?>
    ...
    <div id="data" class="container box">
		<div class="table-responsive">
			...
			<div align="right">
				<button type="button" style="background: #008d;" id="add_button" data-toggle="modal" data-target="#userModal" class="btn btn-info ">Agregar Cooperativa</button>
			</div>
            <div align="left">
			...
			<table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
					<?php
                        $statement = $conn->prepare("SELECT cooperativa.*, usuario.Nombre AS NombreUsuario
                        FROM cooperativa
                        INNER JOIN usuario ON cooperativa.Id_Usuario = usuario.Id_Usuario");
						$statement->execute();
						...
						while ($a < count($result))
                        {
							echo'<tr>
								...
								<td align="center"><a>'.$result[$a]["Ruc"].'</a</td>
                                <td align="center"><a>'.$result[$a]["Direccion"].'</a></td>
								<td align="center"><a>'.$result[$a]["Contacto"].'</a></td>
								<td align="center"> <button type="button" name="update"  id="'.$result[$a]["Id_Cooperativa"].'" class="btn btn-warning btn-xs update"><i type="button"  class="fa fa-edit btn-xs"></i></button> <button type="button" id="'.$result[$a]["Id_Cooperativa"].'" name="delete" class="btn btn-danger btn-xs delete"><i type="button"  class="fa fa-trash btn-xs "></i></button></td>
							</tr>';
							$a ++;
						}
					?>
                        ...
                        <form method="post" id="user_form" enctype="multipart/form-data">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title">Agregar Cooperativa</h4>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-row">
                                    <label>Ruc</label>
                                        <input type="text" name="Ruc" id="Ruc" placeholder="Ruc" class="form-control" maxlength="13" required pattern="[0-9]{13}" title="El Ruc debe contener exactamente 13 dígitos numéricos." />
                                    </div>
                                    <div class="form-row">
                                    ...
                                    <label >Contacto</label>
                                        <input type="text" name="Contacto" id="Contacto" placeholder="Contacto" class="form-control"  required />
                                    </div>
                                    <a style="color:#FFFF;">.</a>
                                </div>
                                <div class="modal-footer">
                                    <input type="hidden" name="Id_Cooperativa" id="Id_Cooperativa" />
...
<script>
    ...
    $('#add_button').click(function() {
        $('#user_form')[0].reset();
        $('.modal-title').text("Agregar Cooperativa");
        $('#action').val("Guardar");
        $('#operation').val("Add");
    });
    ...
    $(document).on('submit', '#user_form', function(event) {
        ...
        if (Nombre != '' && Id_Usuario != '' && Ruc != '' && Direccion != '' && Contacto!= '') {
            $.ajax({
    ...
    $(document).on('click', '.update', function() {
        var Id_Cooperativa = $(this).attr("id");
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.modal-title').text("Modificar Cooperativa");
                $('#Id_Cooperativa').val(Id_Cooperativa);
                $('#action').val("Guardar");
                $('#operation').val("Edit");
            }
        })
